...
export function MigrationPage() {
  ...
  function importDebts() {
    if (!confirm('This permanently rewrites every month’s expenses and adds debts to the registry. It cannot be undone. Make sure you have exported a backup from Settings first. Continue?')) return
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-6">
        ⚠ This permanently rewrites your stored months and cannot be undone. Export a backup from Settings → Data first.
      </p>
      ...
      {candidates.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          ...
          <button onClick={() => { if (!confirm('This permanently rewrites every month’s expenses and cannot be undone. Export a backup from Settings first. Continue?')) return; reclassifyExpenses(); setDone(true) }} className="mt-4 bg-gray-900 text-white text-sm px-4 py-2 rounded-lg hover:bg-gray-700">Reclassify expenses</button>
        </div>
      ) : (
        <>
